function getSensorData() {
    ...
    $.ajax({
        ...
        for (var sensor in laserDataMap) {
            ...
            chartlaser.data.labels.push(sensor);
            chartlaser.data.datasets[0].data.push(avg);
        }
        chartlaser.update();
...
$(document).ready(() => {
    ...
    //파이차트 시작
    var chartArea = document.getElementById('chartlaser').getContext('2d');
    chartlaser = new Chart(chartArea, {
        type: 'pie',
        data: {
            datasets: [{
                label: 'event status',
